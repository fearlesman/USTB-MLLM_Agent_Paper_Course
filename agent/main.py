"""Agent-track entry — same CLI as ``baseline/main.py``; sets default ``AV_SPEAKERBENCH_EVAL_TRACK=agent``."""

import os

# Differentiate result/record/trace filenames from the baseline harness (agent_* prefix).
...
def main():
    ...
    parser.add_argument(
        "--cfg_path", default="eval_configs/video_llama_eval_withaudio.yaml", help="path to configuration file."
    )
    parser.add_argument(
        "--options",
        nargs="+",
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "base limits. Override with env AV_SPEAKERBENCH_MAX_CLIP_BYTES."
        ),
    )
    parser.add_argument(
